"""
PURPOSE: This script will extract specified IOC's from a document or webpage using regular expressions and then put them into a KQL query. An analyst
will now be able to specifiy a document and have an immediate output to paste into Elastic/ELK as oppossed to copy and pasting each IOC from a document or
webpage.
"""
...
class regex:
    ...
    reg_list = [ip_reg, sha_256_reg, sha_1_reg, md5_reg, http_reg, email_reg, domain_reg, path_file_reg]
        # references the function "Elastic_Output" on the code below. 
...
# Similar to the REGEX Class above. Admittedly this reg_iterator() function is a modification of suggestions from colleagues and external resources. 
def reg_iterator(text):
    with open('Elastic_Output.txt', 'w') as file: # when use "w" overwrites as default
...
def ip_check():
    ...
    if len(str(regex.ip_reg)) > 0:  
        ipcheckinput = input("IP address(s) detected, choose field name to search by. source.ip - (1), destination.ip - (2), Neither - (0). : ")
    
    if ipcheckinput == "1":
        return "source.ip:  "  # Return the string "source.ip"
    elif ipcheckinput == "2":
        return "destination.ip:  "  # Return the string "destination.ip"
    else:       
        return ""  # Return an empty string if neither condition is met
    #return ipcheckinput - for testing
#print(ip_check())  - for testing

def initial_question():
    value_map = {
        '1': 'event.dataset:  ',
        '2': 'source.geo.country_name:  ',
        '3': 'event.action:  ' ,
        '4': 'source_port:  ' ,
        '5': 'destination_port:  ' ,
        '6': 'agent.hostname:  ' , 
    ...
    userinput = input("Enter any number combo for desired Elasticsearch ELK/Kibana fields ( Event.Dataset - (1), Source.geo.country_name - (2), Event.Action - (3), Source_port - (4), Destination_port - (5), Agent.Hostname - (6), User.name - (7) or 0 for None ): ")
    
      # Check if the user wants to quit the function
    if "0" in userinput:
        return "" 
        #quit() - for testing
        #return userinput - for testing

    # Validate and process the input
    validchoices = ['1', '2', '3', '4', '5', '6','0']
...
def Elastic_Output(reg): # Initially no values, references the Regex up top. 
    global initial_choices
    counter = 0
    with open('Elastic_Output.txt', 'a') as f: # "a" is for appending the file. # Append vs Write???
        if reg == regex.ip_reg:
        ...
        if len(new_list) == 0:    #This part just writes to a clean up version of the RAW input. 
            ...
            for k in new_list: 
                ...
                if counter == len(new_list)-1: # -1 will give the last item in the list.
                    ...
                    f.write(f'"{k}" OR ') # writes "OR" for IN statements. 
                    counter=counter+1
